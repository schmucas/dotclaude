...
description: "Pause-and-confirm gate between local Databricks development and the real workspace. Use whenever Databricks code (job, pipeline, notebook, app) has passed local checks - written, locally tested, ruff clean - and the next step would be `databricks bundle validate`, `bundle deploy`, running a job/pipeline, or checking results via the Databricks SQL MCP. Does not apply if the user's own request already explicitly asked for deploy/run. For how to actually validate, deploy or run, defer to the vendor databricks-dabs and databricks-jobs skills; this skill only decides whether to ask first."
---
...
job or pipeline, querying results through the Databricks SQL MCP) are two separate
phases. Do not slide from one into the other on your own initiative.
...
4. Check the result via the Databricks SQL MCP (or job run output)

Ask once, covering the whole sequence, e.g.:

> Local dev is done: tests pass, ruff is clean. Want me to validate the bundle,
> deploy to dev, run it, and check the result via SQL MCP?
...
`databricks-dabs` and `databricks-jobs` skills. For querying results, defer to
`databricks-dbsql` / the Databricks SQL MCP. This skill only governs whether to ask
first, not how the commands work.
